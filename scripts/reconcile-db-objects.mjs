...
function groupReferences(references) {
  ...
  return [...groups.values()].sort((a, b) => `${a.kind}:${a.schema}.${a.name}`.localeCompare(`${b.kind}:${b.schema}.${b.name}`));
}
...
export function reconcile(projectRoot, config, outputDir) {
  ...
  fs.writeFileSync(generatedTestAbsolute, generateAuthorizationTestSql(rows, config), "utf8");
  ...
  return { summary, rows, source, migrations, generatedTestRelative };
}
